...
| size    | Produces a single `true` if both expressions have the same number of results. |
| typeof  | Produces a single `true` if the first value of both expressions is the same data type (string, number, boolean, object, array). |
| ntypeof | Produces a single `true` if the first value of both expressions are **not** the same data type (string, number, boolean, object, array). |
| add     | Adds the first value on of the right expression to every value on the left. |
| sub     | Subtracts the first value on of the right expression from every value on the left. |
| mul     | Multiples every value of the left expression by the first value on the right. |
| div     | Divides every value of the left expression by the first value on the right. |
| mod     | Reduces every value of the left expression to the modulo of the first value on the right. |
| pow     | Returns every value of the left expression, to the exponent of the first value on the right. |
| join    | Joins the values on the left side of the expression into a single string, using the first value on the right. |
| split   | Splits every string value on the left using the first value of the right expression as the delimiter |
| keys    | Outputs the keys of any Object, Array, Set or Map received from the left expression. |
| values  | Outputs the values of any Object, Array, Set or Map received from the left expression. |
| abs     | Outputs the absolute value for every value in the left expression. |
| ceil    | Rounds up every value from the left side expression to the nearest whole number |
| floor   | Rounds down every value from the left side expression to the nearest whole number |
| round   | Rounds every value from the left side expression to the nearest whole number |
| min     | Outputs the smallest numeric value from the result of the expression left of the operator |
| max     | Outputs the largest numeric value from the result of the expression left of the operator |
| sum     | Outputs the sum of all numeric values in the result of the expression left of the operator |
| avg     | Outputs the mean average of all numeric value in the result of the expression left of the operator |
| med     | Outputs the median average of all numeric value in the result of the expression left of the operator |
| stddev  | Outputs the standard deviation of all numeric value in the result of the expression left of the operator |
| random  | Outputs a single random value from the results of the expression left of the operator |
| first   | Outputs the first value from the results of the expression left of the operator |
| last    | Outputs the left value from the results of the expression left of the operator |
| unique  | Removes duplicate values from the results of the expression left of the operator |
| sort    | Sorts the results of the expression left of the operator, using a case-insensitive natural sort that ignores punctuation. |
| any     | Produces a single `true` if any of the results of the expression left of the operator are truthy. |
| all     | Produces a single `true` if all of the results of the expression left of the operator are truthy. |
| none    | Produces a single `true` if none of the results of the expression left of the operator are truthy. |
| sizeof  | Outputs the size of every result of the expression left of the operator. For collections this is the number of child elements. For strings, this is the length of the string. |
| count   | Outputs how many results were produced by the expression left of the operator. |
| empty   | Produces a single `true` if the expression left of the operator produced no results. |
